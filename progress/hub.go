...
// Package progress provides the ProgressHub and related types for in-process
// backtest progress broadcasting. It lives in its own package so that both
// the backtest runner and the HTTP handler can import it without creating an
// import cycle.
package progress
...
// Hub manages in-flight run progress state.
...
func NewHub() *Hub {
	return &Hub{runs: make(map[uuid.UUID]*runBroadcaster)}
}

func (h *Hub) broadcaster(runID uuid.UUID) *runBroadcaster {
	h.mu.RLock()
	b, ok := h.runs[runID]
	h.mu.RUnlock()
	if ok {
...
func (h *Hub) Publish(runID uuid.UUID, msg ProgressMessage) {
...
func (h *Hub) Complete(runID uuid.UUID, status, errMsg string) {
...
// LineWriter parses newline-delimited JSON from strategy stdout and
...
// NewLineWriter returns a writer that parses strategy stdout JSON lines
// and publishes progress events to hub.
func NewLineWriter(hub *Hub, runID uuid.UUID) *LineWriter {
	return &LineWriter{hub: hub, runID: runID}
}
